...
class EfficientGlobalPointerNet(nn.Module):
    def __init__(self,model_path,categories_size,head_size,hidden_size,rel):
        ...
        self.globalpointer_rel_tail = EfficientGlobalPointer(rel, head_size, hidden_size)
        self.bert = BertModel.from_pretrained(model_path)
    def forward(self, input_ids, attention_mask, token_type_ids,input_ids_type,attention_mask_type,token_type_ids_type):
        bert_encoder = self.bert(input_ids, attention_mask=attention_mask, token_type_ids=token_type_ids,output_hidden_states=True)
        bert_encoder = bert_encoder.last_hidden_state
        logits_ent = self.globalpointer(bert_encoder, mask = attention_mask)
        logits_tail = self.globalpointer_rel_tail(bert_encoder,mask = attention_mask)
        return logits_ent,logits_tail
